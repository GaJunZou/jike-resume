...
export default class Draggable {
  ...
  bindEventListener() {
    ...
    this.wrap.addEventListener("pointerdown", (e) => {
      ...
      // console.log(getAncestorDom(e.target, this.triggerClass));
      if (getAncestorDom(e.target, this.triggerClass)) {
        ...
        moveTarget.style.top = "-10px";
        moveTarget.style.left = "-10px";
    ...
    this.wrap.addEventListener("pointermove", (e) => {
      if (isOnPointer) {
        ...
        for (let i = 0; i < this.list.length; i++) {
          ...
          if (ele !== dragTarget) {
            if (rect.x < e.x && e.x < rect.x + rect.height && rect.y < e.y && e.y < rect.y + rect.width) {
              let swapNode = index > dragTargetIndex ? ele.nextElementSibling : ele;
              this.wrap.insertBefore(dragTarget, swapNode);
              dragTargetIndex = index;
              console.log("交换");
              this.getRect();
              dragTarget.style.transition = "none";
              dragTarget.style.transform = `translate3d(${ele.offsetLeft - dragTarget.offsetLeft}px, ${
                ele.offsetTop - dragTarget.offsetTop
              }px, 0)`;
              ...
              ele.style.transform = `translate3d(${dragTarget.offsetLeft - ele.offsetLeft}px, ${
                dragTarget.offsetTop - ele.offsetTop
              }px, 0)`;
              ele.offsetLeft; // 触发重绘
